...
def predict(image):
    ...
    payload = {"image_vector": image_data.tolist()}  # Convert NumPy array to list

    try:
        response = requests.post(url, json=payload)
        if response.status_code == 200:  # Check response
            print("Prediction:", response.json())  # Display result
            messagebox.showinfo("Result", f"Digit: {response.json()['prediction']}")
        else:
            print("Error:", response.text)  # Print error message
...
class DrawingApp:
    ...
    def predict_image(self):
        ...
        predict(image_data) # predict function which sends a request to the server
